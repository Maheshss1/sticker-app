...
public class EntryActivity extends BaseActivity {
    ...
    public static final String TAG = "tewfwefag";
    ...
    private void showStickerPack(ArrayList<StickerPack> stickerPackList) {
        ...
        if (stickerPackList.size() > 1) {
            final Intent intent = new Intent(EntryActivity.this, StickerPackListActivity.class);intent.putParcelableArrayListExtra(StickerPackListActivity.EXTRA_STICKER_PACK_LIST_DATA, stickerPackList);
            ...
            database.getReference().get().addOnSuccessListener(new OnSuccessListener<DataSnapshot>() {
                ...
                public void onSuccess(DataSnapshot dataSnapshot) {
                    ...
                    for(DataSnapshot snapshot: dataSnapshot.getChildren()) {
                        StickerPack stickerPack = snapshot.getValue(StickerPack.class);
                        stickerPacks.add(stickerPack);
                    }
                    StickerContentProvider.setStickersPackList(stickerPacks);
                    Log.d(TAG, "onSuccess23: "+stickerPacks);
                    final EntryActivity entryActivity = contextWeakReference.get();
                    if (entryActivity != null) {
                        entryActivity.showStickerPack(stickerPacks);
                    }
                }
